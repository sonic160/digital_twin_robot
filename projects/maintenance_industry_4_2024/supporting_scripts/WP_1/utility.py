...
def read_all_test_data_from_path(
    base_dictionary: str, pre_processing: callable = None, is_plot=True
) -> pd.DataFrame:
    """## Description
    ...
    path_list.remove("Test conditions.xlsx")
    ...
    for selected_sequence_idx in path_list:
        ...
        print(
            "{}: {}\n".format(
                ...
                df_test_conditions[
                    df_test_conditions["Test id"] == selected_sequence_idx
                ]["Description"],
            )
        )
...
def read_all_csvs_one_test(
    folder_path: str, test_id: str = "unknown", pre_processing: callable = None
) -> pd.DataFrame:
    """## Description
...
def run_cross_val(
    reg_mdl, df_x, y, n_fold=5, threshold=3, window_size=0, single_run_result=True
):
    """## Description
    ...
    return pd.DataFrame(
        data=perf, columns=["Max error", "RMSE", "Exceed boundary rate"]
    )
...
def model_pef(y_tr, y_test, y_pred_tr, y_pred):
    ...
    print(
        "Training performance, mean root square error is: "
        + str(mean_squared_error(y_tr, y_pred_tr, squared=False))
    )
    print(
        "Training performance, residual error > 3: "
        + str(sum(abs(y_tr - y_pred_tr) > 3) / y_tr.shape[0] * 100)
        + "%"
    )

    print("Prediction performance, max error is: " + str(max_error(y_pred, y_test)))
    print(
        "Prediction performance, mean root square error is: "
        + str(mean_squared_error(y_pred, y_test, squared=False))
    )
    print(
        "Prediction performance, percentage of residual error > 3："
        + str(sum(abs(y_pred - y_test) > 3) / y_test.shape[0] * 100)
        + "%"
    )
