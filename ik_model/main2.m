...
for k = 1:numberofshapes
    ...
    datapoint = zeros(len_time_series,3);
    disp("Equations de la forme");
    disp(shapes_dict.(shape));
    for t = 1:len_time_series
        datapoint(t, :) =[shapes_dict.(shape).xequation(t), shapes_dict.(shape).yequation(t), shapes_dict.(shape).zequation(t)];
        spline(t,:)  = datapoint(t, :);
        if t>1 
    ...
    for j=0:6   %on mettra 64 plus tard si on le veut
        ...
        dataset=[dataset,jdatapoint];%on ajoute jdatapoin au dataset
    end
